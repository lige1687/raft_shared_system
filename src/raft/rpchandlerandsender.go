...
func (rf *Raft) AppendEntries(args *AppendEntriesArgs, reply *AppendEntriesReply) {
	...
	if args.PrevLogIndex < rf.lm.FirstIndex() {
		reply.Term, reply.Success = rf.currentTerm, false
		return
	}
	...
	if !rf.LogMatched(args.PrevLogIndex, args.PrevLogTerm) {
		reply.Term, reply.Success = rf.currentTerm, false
		lastLogIndex := rf.lm.LastIndex()
		// find the first index of the conflicting term
		if lastLogIndex < args.PrevLogIndex {
			// the last log index is smaller than the prevLogIndex, then the conflict index is the last log index
			reply.XIndex, reply.XTerm = lastLogIndex+1, -1
		} else {
			firstLogIndex := rf.lm.FirstIndex()
			// find the first index of the conflicting term
			index := args.PrevLogIndex
			for index >= firstLogIndex && rf.lm.logs[index-firstLogIndex].Term == args.PrevLogTerm {
				index--
			}
			reply.XIndex, reply.XTerm = index+1, args.PrevLogTerm
		}
		return
	}
	...
	for index, entry := range args.Entries {
		...
		if entry.Index-firstLogIndex >= len(rf.lm.logs) || rf.lm.logs[entry.Index-firstLogIndex].Term != entry.Term {
			rf.lm.logs = shrinkEntries(append(rf.lm.logs[:entry.Index-firstLogIndex], args.Entries[index:]...))
			rf.persist()
			break
		}
	}
	...
	newCommitIndex := min(args.LeaderCommit, rf.lm.LastIndex())
...
func (rf *Raft) genInstallSnapshotArgs() *InstallSnapshotArgs {
	firstLog := rf.lm.GetEntry(rf.lm.FirstIndex())
